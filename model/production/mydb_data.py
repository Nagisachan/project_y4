...
class DB:
    ...
    def get_all_tag(self):
        self.cur.execute("select concat(category_id,'-',item) as tag, name as name from tag_category_item where status='A'")
        return self.cur.fetchall()

    def read_text_tag(self):
        self.cur.execute("select concat(t.file_id,'-',t.paragraph_id) as id, c.content as text, GROUP_CONCAT(t.tag) as tag from tag t left join content c on t.file_id=c.file_id and t.paragraph_id=c.paragraph_id where t.status='A' and t.type='M' group by t.file_id,t.paragraph_id, c.content")
