<?php
	...
	else
	{
		...
		if($out['count'] == "1")
		{
			...
			$sql = "UPDATE tb_food SET fat = ". $changeTo ." WHERE foodID = '". $foodID ."'";
			if (mysqli_query($con, $sql))
			{
				$GCMID = mysqli_real_escape_string($con, $_SESSION['username']);
				$sql = "INSERT INTO tb_user_log (userLogNo, userID, actionCode, foodID, description) VALUES((SELECT CASE COUNT(*) WHEN 0 THEN 0 ELSE MAX(ul.userLogNo) END + 1 FROM tb_user_log ul), '" . $GCMID . "', (SELECT actionCode FROM tb_action_log WHERE actionName = 'Edit'), '" . $foodID . "', 'Fat = " . $changeTo . "')";
				$result = mysqli_query($con, $sql);
				echo "true";
			}
			else
