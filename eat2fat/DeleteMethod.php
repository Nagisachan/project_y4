<?php
	...
	else
	{
		...
		if($out['count'] == "1")
		{
			...
			$sql = "SELECT methodName FROM tb_method WHERE methodID ='". $methodID ."'";
			$result = mysqli_query($con, $sql);
			$out = $result->fetch_array(MYSQLI_ASSOC);
			$methodName = $out['methodName'];

			$sql = "DELETE FROM tb_method WHERE methodID ='". $methodID ."'";
			if (mysqli_query($con, $sql))
			{
				$GCMID = mysqli_real_escape_string($con, $_SESSION['username']);
				$sql = "INSERT INTO tb_user_log (userLogNo, userID, actionCode, description) VALUES((SELECT CASE COUNT(*) WHEN 0 THEN 0 ELSE MAX(ul.userLogNo) END + 1 FROM tb_user_log ul), '" . $GCMID . "', (SELECT actionCode FROM tb_action_log WHERE actionName = 'Delete'), '" . $methodID . " " . $methodName . "')";
				$result = mysqli_query($con, $sql);
				echo "true";
			}
			else
